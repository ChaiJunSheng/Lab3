...
def get_employees_by_dept(department):
    result = []
    for item in employee_data:
        if  item['department'] == department: #['department'] and ["department"] is same but if () then wrong
            result.append(item)
...
def display_main_menu():
    ...
    if option == '1':
        ...
        employee_info = get_employees_by_age_range(age_lower_limit, age_upper_limit) #employee is result
